...
def test_klippertouch_service_is_independent_systemd_unit() -> None:
    ...
    assert "Environment=QSG_INFO=1" in service
...
def test_klippertouch_eglfs_service_is_isolated_experiment() -> None:
    service = Path("deploy/klippertouch-eglfs.service").read_text(encoding="utf-8")

    assert "Description=KlipperTouch EGLFS Touchscreen GUI Experiment" in service
    assert "User=tope" in service
